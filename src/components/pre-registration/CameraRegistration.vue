...
const sendImage = async () => {
    imageCaptured.value = true;
    // Comprimir la imagen
    const compressedFile = await imageCompression.getFilefromDataUrl(capturedImage.value, {
        maxSizeMB: 0.7,
    ...
    <div class="relative flex-col items-center justify-center">
        <div class="flex items-center justify-center">
            ...
            <canvas
                ...
                :class="{ 'absolute top-0 left-0': !imageCaptured }"></canvas>
        </div>

        <div class="flex items-center justify-center">
            <a-button class="w-20 h-20 mt-2 text-3xl" @click="captureImage" v-if="!imageCaptured">
                <CameraOutlined />
            </a-button>
        </div>
        <div class="flex items-center justify-center mt-10 space-x-3" v-if="imageCaptured && !successSend">
            <a-button class="w-32 h-20" @click="sendImage">Enviar Foto</a-button>
            <a-button class="w-32 h-20" @click="retakeImage">Tomar Otra Foto</a-button>
        </div>
    </div>
</template>
<style scoped>
